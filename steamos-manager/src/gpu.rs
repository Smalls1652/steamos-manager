...
pub(crate) async fn gpu_performance_level_driver() -> Result<Box<dyn GpuPerformanceLevelDriver>> {
    // First try AMD GPU
    if let Ok(_) = find_hwmon(AMDGPU_HWMON_NAME).await {
    ...
    for card_num in 0..4 {
        ...
        if try_exists(card_path.join(IntelGpuConfig::I915.min_freq)).await? {
            ...
            return Ok(Box::new(IntelGpuPerformanceLevelDriver {
                card_path: card_path,
                config: IntelGpuConfig::I915,
            }));
        } else if try_exists(card_path.join(IntelGpuConfig::XE.min_freq)).await? {
            debug!("Found Intel Xe GPU at {}", card_path.display());
            return Ok(Box::new(IntelGpuPerformanceLevelDriver {
                card_path: card_path,
                config: IntelGpuConfig::XE,
            }));
        }
    }
...
impl IntelGpuPerformanceLevelDriver {
    ...
    async fn get_min_clocks(&self) -> Result<u32> {
        self.read_freq(self.card_path.join(self.config.min_freq))
            .await
    }

    async fn set_min_clocks(&self, clocks: u32) -> Result<()> {
        self.write_freq(self.card_path.join(self.config.min_freq), clocks)
            .await
    }

    async fn get_max_clocks(&self) -> Result<u32> {
        self.read_freq(self.card_path.join(self.config.max_freq))
            .await
    }

    async fn set_max_clocks(&self, clocks: u32) -> Result<()> {
        self.write_freq(self.card_path.join(self.config.max_freq), clocks)
            .await
    }
}
...
impl GpuPerformanceLevelDriver for IntelGpuPerformanceLevelDriver {
    ...
    async fn get_clocks_range(&self) -> Result<RangeInclusive<u32>> {
        ...
        let min_val = self
            .read_freq(self.card_path.join(self.config.range_min))
            .await?;
        let max_val = self
            .read_freq(self.card_path.join(self.config.range_max))
            .await?;
